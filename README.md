# 自然派语法汇总
# 更新2025.09.29
放到pip上了，以后只需要 `pip install ziranpai` 就可以安装，安装完直接 `自然派` 就可以运行了。增加 `自导入 自然派文件 模块名`,可以导入自然派写的代码文件，通过`模块名#函数`引用模块函数，`派导入 xxx.py 模块名`，`模块名#函数`导入python模块，应该也可以导入python任意包如numpy等。
# 更新2025.09.22
更新最新版为 密自然派 。直接 `python3 密自然派` 就可以运行。增加 多行输入 函数，更新了类以及#调用数组、字典和类的方法：
```bash
