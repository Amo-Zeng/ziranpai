...
def tongji(l):
    ...
    for i in l:
        if i in you.keys():
            you[str(i)]=you[str(i)]+1
        else:
            you[str(i)]=1
    return dict(sorted(you.items(),key=operator.itemgetter(1)))
def jianming(d):
    return list(d.keys())
def jianzhi(d):
...
def hangbiao(d):
    #print("a",d.loc[1,:])
    biao=[list(d[k]) for k in list(d.keys())]
    #for i in np.transpose(biao):
    #    print(len(i))
    ##print(biao)
    #biao=[]
    #for i in d[list(d.keys())[0]]:
    #    biao.append([list(d[k])[i] for k in list(d.keys())])
    #print(biao)
    return np.transpose(biao)
#a=pd.read_excel("./excel/1.xls")
#print(list(a))
#quit()
xwz1={"读表":pd.read_excel,"统计":tongji,"键名":jianming,"键值":jianzhi,"行表":hangbiao}
xwz2={}
xwz3={}
xwz4={}
